...
if st.session_state.page == "🔄 Обучение модели":
    ...
    if st.session_state.uploaded_data is not None:
        ...
        # Команды для выбора account_id
        radiant_ids = st.multiselect("Выберите Account ID для Radiant", 
                                      data[data['team'] == 'Radiant']['account_id'].unique().tolist())
        dire_ids = st.multiselect("Выберите Account ID для Dire", 
                                   data[data['team'] == 'Dire']['account_id'].unique().tolist())
        if st.button("🔮 Получить предсказания"):
            st.write("Предсказания для команды Radiant:")
            for account_id_input in radiant_ids:
            ...
            for account_id_input in dire_ids:
                account_data = data[data['account_id'] == account_id_input]
                if account_data.empty:
            ...
            st.error("Выберите хотя бы один Account ID для предсказаний.")
    else:
